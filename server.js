...
const SongJinn = require('./src/auto/game').SongJinn;
const server = Server({ games: [SongJinn], generateCredentials: () => uuidv4() });
...
app.use(
    ratelimit({
        ...
        max: 25,
        whitelist: (ctx) => {
            return !ctx.path.includes(`games/${SongJinn.name}`);
        },
    })
);
